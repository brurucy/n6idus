...
function flattenBuckets<K>(
  indexedSortedArraySet: IndexedSortedArraySet<K>
): Array<K> {
  let out = new Array<K>();
...
describe("IndexedSortedArraySet", () => {
  ...
  describe("locate", () => {
    ...
    describe("correctly checks for invalid boundaries", () => {
      const extendedIndexedSortedArraySet = new ExtendedIndexedSortedArraySet(
        2
      );
      extendedIndexedSortedArraySet.pushExtended(1);

      const cases: Array<[number, undefined]> = [
        [-1, undefined],
        [1, undefined],
      ];
      test.each(cases)("locating %d returns %d", (nth, location) => {
        expect(extendedIndexedSortedArraySet.locateExtended(nth)).toEqual(
          location
        );
      });
    });
    describe("returns the expected two-level position", () => {
      ...
      test.each(cases)(
        "%d is in bucket %d at the %d position",
        (nth, value, [expectedBucketNumber, expectedPositionInBucket]) => {
          const locatingAttempt =
            extendedIndexedSortedArraySet.locateExtended(nth);
          if (locatingAttempt != undefined) {
            ...
            expect(
              extendedIndexedSortedArraySet.buckets[locatingAttempt[0]].select(
                locatingAttempt[1]
              )
            ).toEqual(value);
          } else {
  ...
  describe("make cursor", () => {
    ...
    describe("traverses the entire structure forward by position", () => {
      ...
      test.each(enumeratedSortedArray)(
        "expected %d got %d",
    ...
    describe("traverses the entire structure backwards by position", () => {
      ...
      const initiallyBackwardsCursor = indexedSortedArraySet.makeCursor(
        indexedSortedArraySet.length - 1,
        false
      );
      test.each(invertedEnumeratedSortedArray.reverse())(
        "expected %d got %d",
        (expectedIndex, expectedValue) => {
  ...
  describe("symmetricDifference", () => {
    describe("satisfies the definition of symmetricDifference", () => {
      ...
      test.each(cases)("%s Δ %s = %s", (L, R, LDR) => {
        expect(flattenBuckets(L.symmetricDifference(R))).toEqual(
          flattenBuckets(LDR)
        );
      });
    });
  ...
  describe("reduce", () => {
    describe("works as expected", () => {
      ...
      expect(
        ...
        flattenBuckets(indexedSortedArraySet).reduce(
          (
            previousValue: number,
            currentValue: number,
          ) => {
