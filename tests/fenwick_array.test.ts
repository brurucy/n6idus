import { FenwickArray} from "../src/data_structures/fenwick_array";
...
describe("FenwickArray", () => {
  ...
  describe("prefixSum", () => {
    describe("calculates the expected prefix sum", () => {
      ...
      const cases: Array<[number, number | undefined]> = [[0, 0], [1, 1], [2, 7], [3, 10], [4, 19], [5, 21], [6, undefined]]
  ...
  describe("increaseLength", () =>{
    describe("correctly adjusts the partial sums", () => {
      ...
      const cases: Array<[number, number]> = [[0, 2], [1, 8], [2, 3], [3, 20], [4, 2]]
  ...
  describe("decreaseLength", () => {
    describe("correctly adjusts the partial sums", () => {
      ...
      const cases: Array<[number, number]> = [[0, 0], [1, 6], [2, 3], [3, 18], [4, 2]]
      test.each(cases)("index %d expected value %d", (index, value) => {
        expect(fenwickArray.innerStructure[index]).toEqual(value);
      })
    });
  ...
  describe("indexOf", () => {
    describe("returns the expected position of the given prefix sum", () => {
      ...
      const cases: Array<[number, number]> = [[0, 0], [1, 6], [2, 9], [3, 18], [4, 20]]
